...
void alsa_init()
{
    ...
    if (err < 0) 
    {
        writelog(ERR, "Cannot subscribe to alsa events: %s\n", snd_strerror(err));
        snd_ctl_close(ctl);
        exit(FAIL);
    }
...
void alsa_close()
{
    ...
    if (err < 0) 
    {
        writelog(ERR, "Cannot close alsa control: %s\n", snd_strerror(err));
        exit(FAIL);
    }
}
...
int alsa_getrate() 
{
    ...
    do 
    {
        ...
        if (err < 0) 
	    // Most likely alsa control was closed by signal handler
	    // after notification of DAC availability,
    ...
    if (err < 0) 
    {
        writelog(ERR, "Cannot read event element: %s\n", snd_strerror(err));
        exit(FAIL);
    }
